...
<script type="text/javascript">
	$('document').ready(function(){
		...
		$('#scatedul').click(function(){
			...
			var gu_scate = $('input[name="gu_scate"]').val();
  ...
  ${pageNumber}
	<div class="col-lg-12">
		<div class="bs-component">
		 ...
		 <form action="adminScateUpdate.cate" method="post" align="center">
		 	<input type="hidden" name="scatenum" value="${scateBean.scatenum}">
		 	<input type="hidden" name="gu_scate" value="${scateBean.scate}">
							...
							<select class="form-select" name="bcate">
                      	 		<option value=""> 대분류명을 선택하세요 </option>
		                            <c:forEach var="clist" items="${cateList }">
		                                <option value="${clist.bcate }" <c:if test="${clist.bcate eq scateBean.bcate}"> selected </c:if> > 
		                                   ${clist.bcate }
		                                </option>
		                            </c:forEach>
							</select>
